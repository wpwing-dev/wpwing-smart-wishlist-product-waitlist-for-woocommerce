...
class AdminSettings {
	...
	public function register_submenu(): void {
		add_submenu_page(
			'wpwing',
			__( 'WPWing Settings', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			__( 'Settings', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			'manage_woocommerce',
			self::PAGE_SLUG,
	...
	public function register_settings(): void {
		...
		add_settings_section(
			'wpwing_wl_features',
			__( 'Features', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			static function (): void {
				echo '<p>' . esc_html__( 'Disable a feature to hide its frontend UI. Existing data is preserved.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ) . '</p>';
			},
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			'enable_wishlist',
			__( 'Enable Wishlist', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			array( $this, 'field_checkbox' ),
			self::PAGE_SLUG,
			'wpwing_wl_features',
			array(
				'key'         => 'enable_wishlist',
				'description' => __( 'Show the wishlist toggle button on product pages and the [wpwing_wishlist] shortcode output.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			)
		);
		add_settings_field(
			'wishlist_page_id',
			__( 'Wishlist Page', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			array( $this, 'field_page_select' ),
			self::PAGE_SLUG,
			'wpwing_wl_features',
			array(
				'key'         => 'wishlist_page_id',
				'description' => __( 'The page that displays your customers\' saved products via the [wpwing_wishlist] shortcode. Used for the nav badge link and count shortcode.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			)
		);
		add_settings_field(
			'enable_waitlist',
			__( 'Enable Waitlist', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			array( $this, 'field_checkbox' ),
			self::PAGE_SLUG,
			'wpwing_wl_features',
			array(
		...
		add_settings_section(
			...
			__( 'Notification Email — Branding', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			static function (): void {
				echo '<p>' . esc_html__( 'Leave a field blank to fall back to the WordPress site name and admin email.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ) . '</p>';
			},
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			'from_name',
			__( 'From name', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			array( $this, 'field_text' ),
			self::PAGE_SLUG,
			'wpwing_wl_email_branding',
			array(
		...
		add_settings_field(
			'from_email',
			__( 'From email', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			array( $this, 'field_text' ),
			self::PAGE_SLUG,
			'wpwing_wl_email_branding',
			array(
		...
		add_settings_field(
			'reply_to',
			__( 'Reply-To email', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			array( $this, 'field_text' ),
			self::PAGE_SLUG,
			'wpwing_wl_email_branding',
			array(
				...
				'type'        => 'email',
				'placeholder' => (string) get_option( 'admin_email' ),
				'description' => __( 'Where customer replies should go. Defaults to the From email.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			)
		);

		add_settings_section(
			'wpwing_wl_email_template',
			__( 'Notification Email — Template', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			static function (): void {
				echo '<p>' . esc_html__( 'Supported placeholders: {product_name}, {product_url}, {unsubscribe_url}, {store_name}. Leave a field blank to use the default.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ) . '</p>';
			},
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			...
			__( 'Subject', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			array( $this, 'field_text' ),
			self::PAGE_SLUG,
			'wpwing_wl_email_template',
			array(
		...
		add_settings_field(
			'email_body_template',
			__( 'Body (HTML)', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			...
			array(
				...
				'description' => __( 'The HTML body is wrapped in your WooCommerce email header and footer.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			)
		);

		add_settings_section(
			'wpwing_wl_retention',
			__( 'Data Retention', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			'__return_false',
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			...
			__( 'Guest wishlist retention (days)', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			...
			array(
				...
				'min'         => 1,
				'max'         => 3650,
				'description' => __( 'Guest wishlist rows older than this are deleted by the weekly cleanup job. Logged-in user wishlists are never auto-deleted.', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
			)
		);
	}
	...
	public function render_page(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'WPWing Smart Wishlist & Product Waitlist Settings', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ); ?></h1>
	...
	public function field_page_select( array $args ): void {
		...
		wp_dropdown_pages(
			array(
				...
				'show_option_none'  => esc_html__( '— Select a page —', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' ),
				'option_none_value' => '0',
			)
		);
		...
		if ( $value > 0 ) {
			...
			if ( $page_url ) {
				printf(
					...
					esc_url( $page_url ),
					esc_html__( 'View page', 'wpwing-smart-wishlist-product-waitlist-for-woocommerce' )
				);
			}
		}
